...
def append_to_database(new_data):
    sheet_db = connect_gsheet("Database3")
...
def main_app():
    ...
    Year = st.selectbox("Select a year", df4['Year (YYYY)'].sort_values().unique())
    month_order = ["January", "February", "March", "April", "May", "June","July", "August", "September", "October", "November", "December"]
    months = df4[df4['Year (YYYY)'] == Year]['Month'].unique()
    ...
    filtered_form = df4[(df4['Month'] == Month) & (df4['Year (YYYY)'] == Year)].sort_values(by='Timestamp', ascending=False)
    ...
    exists = ((df3['Year'] == Year) & (df3['Month'] == Month)).any()
    if not exists:
        st.warning(f"No data in Database3 for {Month} {Year}. You can approve to add it.")
        # Pretty table layout
        for idx, row in filtered_form.iterrows():
            row_exists = (
                (df3["Timestamp"] == str(row['Timestamp'])) &
                (df3["Upload"] == row["Upload"]) &
                (df3["Email Address"] == row["Email Address"])
            ).any()
        ...
        filtered_df = df3.loc[(df3['Month'] == Month) & (df3['Year'] == Year), ["Timestamp", "Upload", "Email Address"]]
